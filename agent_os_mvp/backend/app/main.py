...
def agent_board(run_dir: Path) -> dict[str, list[dict[str, Any]]]:
    ...
    if meeting_path.exists():
        assignments = load_json(meeting_path).get("task_assignments", [])
    assigned_roles = set()
    for item in assignments:
        ...
        if raw_state == "SUCCESS" or raw_state == "ACCEPTED":
            state = "done"
...
def detail_payload(run_dir: Path) -> dict[str, Any]:
    ...
    artifact_path = ai_dir(run_dir) / "artifact_verify_report.json"
    reviewer_path = ai_dir(run_dir) / "reviewer_verdicts.json"
    claim_path = ai_dir(run_dir) / "subagent_claim_ledger.json"
    watchdog_path = ai_dir(run_dir) / "watchdog_report.json"
    memory_path = ai_dir(run_dir) / "main_agent_memory_guard_report.json"
    board = agent_board(run_dir)
    counts = {key: len(value) for key, value in board.items()}
    return {"run_id": run_dir.name, "overall_status": status_of(report), "report": report, "agent_state_board": board, "agent_counts": counts, "artifact_verify": load_json(artifact_path) if artifact_path.exists() else {}, "reviewer": load_json(reviewer_path) if reviewer_path.exists() else {}, "claim_ledger": load_json(claim_path) if claim_path.exists() else {}, "watchdog": load_json(watchdog_path) if watchdog_path.exists() else {}, "memory_guard": load_json(memory_path) if memory_path.exists() else {}}
...
def monitor() -> dict[str, Any]:
    ...
    recent = []
    for run_dir in dirs[:20]:
        report = report_for(run_dir)
        recent.append({"run_id": run_dir.name, "overall_status": status_of(report), "spec_id": report.get("spec_id"), "updated_at": run_dir.stat().st_mtime})
